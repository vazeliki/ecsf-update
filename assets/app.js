    ...
    .map((r) => `
      <div class="role-card role-card-ecsf" data-slug="${roleSlug(r)}">
        ...
        <p class="role-card-summary">
          ${esc(r.summary || "")} <span class="role-card-more">[more]</span>
        </p>
      </div>`)
    .join("");
  ...
  grid.innerHTML = items
    .map((s) => `
      <div class="skill-card" data-id="${s.id.toLowerCase()}">
        ...
        <h3>${esc(s.name)}</h3>
        <p class="muted" style="font-size:13px;margin:0 0 12px;">${esc(s.description || "")}</p>
...
function renderRoleDetail(slug) {
  ...
  document.getElementById("role-detail-summary").textContent = role.summary || "";
